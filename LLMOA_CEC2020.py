...
def main(dim):
    ...
    for i in range(len(CEC2020)):
        ...
        for j in range(Trials):
            ...
            Trace = LLMHHA(CEC2020[i])
            All_Trial_Best.append(Trace)
        np.savetxt("./LLMHHA_Data/CEC2020/F" + str(i + 1) + "_" + str(dim) + "D.csv", All_Trial_Best, delimiter=",")


if __name__ == "__main__":
    if os.path.exists('LLMHHA_Data/CEC2020') == False:
        os.makedirs('LLMHHA_Data/CEC2020')
    Dims = [30, 50]
    for dim in Dims:
        main(dim)
